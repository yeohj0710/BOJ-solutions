...
int32_t main() {
    ...
    for(int t=1; t<=T; t++) {
        ...
        for(int i=0; i<N; i++) {
            ...
            vector<pair<int, int>> v = po.factorization(x);

            for(int j=0; j<v.size(); j++) m1[v[j].first] += v[j].second;
        }
        ...
        for(int i=0; i<M; i++) {
            ...
            for(int j=0; j<v.size(); j++) m2[v[j].first] += v[j].second;
        }

        int x = 1, y = 1;

        for(int i=0; i<p.size(); i++) {
            if(m1[p[i]] == m2[p[i]]) continue;

            int dif = abs(m1[p[i]] - m2[p[i]]);
            int mul = 1;

            for(int j=0; j<dif; j++) mul *= p[i];

            if(m1[p[i]] > m2[p[i]]) x *= mul;
            else y *= mul;
        }
