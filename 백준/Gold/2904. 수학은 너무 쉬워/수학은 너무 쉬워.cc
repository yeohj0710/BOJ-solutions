...
int32_t main() {
    ...
    for(int i=0; i<N; i++) {
        ...
        vector<pair<int, int>> w = po.factorization(v[i]);

        for(int j=0; j<w.size(); j++) {
            if(m[w[j].first] == 0) u.push_back(w[j].first);
    ...
    for(int i=0; i<u.size(); i++) {
        ...
        for(int j=0; j<x; j++) y *= u[i];

        ans *= y;

        for(int j=0; j<N; j++) {
            vector<pair<int, int>> w = po.factorization(v[j]);

            bool chk = false;
            for(int k=0; k<w.size(); k++)
                if(w[k].first == u[i]) {
                    chk = true;
                    cnt += max(x - w[k].second, (int)0);
                }
