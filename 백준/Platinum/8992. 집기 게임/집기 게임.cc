...
main() {
    ...
    while(T--) {
        ...
        vector<line> v(N+1), u(M+1);

        for(int i=1; i<=N; i++) {
            cin >> v[i].bx >> v[i].by >> v[i].ex >> v[i].ey >> v[i].w;

            if(v[i].bx > v[i].ex) swap(v[i].bx, v[i].ex);
        }

        for(int i=1; i<=M; i++) {
            cin >> u[i].bx >> u[i].by >> u[i].ex >> u[i].ey >> u[i].w;

            if(u[i].by > u[i].ey) swap(u[i].by, u[i].ey);
        }
        ...
        for(int i=1; i<=N; i++) f.edge(sour, i, 1, 0);
        for(int i=1; i<=M; i++) f.edge(N+i, sink, 1, 0);

        for(int i=1; i<=N; i++)
            for(int j=1; j<=M; j++)
                if(v[i].bx < u[j].bx && u[j].bx < v[i].ex
                   && u[j].by < v[i].by && v[i].by < u[j].ey) f.edge(i, N+j, 1, -(v[i].w * u[j].w));
