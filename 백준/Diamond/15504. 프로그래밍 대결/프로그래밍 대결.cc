...
signed main(){
    ...
    MCMF f; f.adj.resize(N*2 + 3);

    int sour = N*2 + 1, sink = N*2 + 2;

    struct s { int a, b, c; }; vector<s> v(N+1);

    int mx = 0;

    for(int i=1; i<=N; i++) cin >> v[i].a, mx = max(mx, v[i].a);
    ...
    for(int i=1; i<=N; i++)
        if(v[i].a == mx) isMax[i] = true;

    for(int i=1; i<=N; i++) {
        if(isMax[i]) f.edge(sour, i, v[i].c, v[i].b);
        else {
            f.edge(sour, i, v[i].c - 1, v[i].b);
            f.edge(N+i, sink, 1, v[i].b);
        }

        for(int j=1; j<=N; j++)
            if(v[i].a > v[j].a) f.edge(i, N+j, 1, -(v[i].a ^ v[j].a));
    }

    cout << -f.mcmf(sour, sink, N-1) << "\n";
}
